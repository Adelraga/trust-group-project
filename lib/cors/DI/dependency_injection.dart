...
void initGetIt() {
  ...
  getIt.registerLazySingleton<ShowKarkastanRequirePaperCubit>(
      () => ShowKarkastanRequirePaperCubit(getIt()));
  getIt.registerLazySingleton<KarkastanRequirePaperRepo>(
      () => KarkastanRequirePaperRepo(getIt()));

  // Get student images in whoweare page
  getIt.registerLazySingleton<StudentImagesCubit>(
      () => StudentImagesCubit(getIt()));
  getIt.registerLazySingleton<StudentImagesRepo>(
      () => StudentImagesRepo(getIt()));

  // Get karkastan common question
  getIt.registerLazySingleton<GetCommonQuestionCubit>(
      () => GetCommonQuestionCubit(getIt()));
  getIt.registerLazySingleton<KarkastanCommonQuestionRepo>(
      () => KarkastanCommonQuestionRepo(getIt()));

  // Get russain pdfs
  getIt.registerLazySingleton<GetRussainPdfsCubit>(
      () => GetRussainPdfsCubit(getIt()));
  getIt.registerLazySingleton<TechnicalPdfCubit>(
      () => TechnicalPdfCubit(getIt()));
  getIt.registerLazySingleton<RussainIraqPdfCubit>(
      () => RussainIraqPdfCubit(getIt()));

  // getkarkastanUniversitiesPrices
  getIt.registerLazySingleton<KarkastanUniversitiesPricesCubit>(
      () => KarkastanUniversitiesPricesCubit(getIt()));
